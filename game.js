...
function createPlayerBike() {
  ...
  const trail = new THREE.Mesh(
    ...
    new THREE.MeshBasicMaterial({ color: 0x22d3ee, transparent: true, opacity: 0.62, side: THREE.DoubleSide })
  );
  trail.rotation.x = -Math.PI / 2;
  trail.position.set(0, 0.06, -3.15);
  group.add(trail);
  ...
  function clearVisuals() {
    while (visuals.children.length) {
      visuals.remove(visuals.children[0]);
    }
  }
  ...
  function configureModel(root, scalar, y = 0, zRot = Math.PI) {
    root.scale.setScalar(scalar);
    root.position.set(0, y, 0.04);
    root.rotation.y = zRot;
    root.traverse((node) => {
      ...
      const baseColor = srcMat?.color ? srcMat.color.clone() : new THREE.Color(0x6ee7ff);
      const boosted = baseColor.clone().multiplyScalar(0.8).lerp(new THREE.Color(0x0ea5e9), 0.25);
      node.material = new THREE.MeshStandardMaterial({
        color: boosted,
        roughness: 0.28,
        metalness: 0.72,
        emissive: baseColor.clone().lerp(new THREE.Color(0x22d3ee), 0.38),
  ...
  function buildFallbackBike() {
    ...
    bodyPath.moveTo(-0.11, -0.82);
    bodyPath.lineTo(0.23, -0.62);
    bodyPath.lineTo(0.27, 0.18);
    bodyPath.lineTo(0.12, 0.72);
    bodyPath.lineTo(-0.17, 0.82);
    bodyPath.lineTo(-0.25, 0.14);
    bodyPath.lineTo(-0.22, -0.56);
    bodyPath.closePath();

    const bodyGeo = new THREE.ExtrudeGeometry(bodyPath, { depth: 0.28, bevelEnabled: true, bevelSegments: 3, bevelSize: 0.04, bevelThickness: 0.04 });
    const body = new THREE.Mesh(bodyGeo, chassisMat);
    body.rotation.x = Math.PI / 2;
    body.position.set(0, 0.52, 0);

    const canopy = new THREE.Mesh(new THREE.CapsuleGeometry(0.14, 0.74, 8, 14), darkMat);
    canopy.rotation.z = Math.PI / 2;
    canopy.position.set(0, 0.7, -0.02);

    const nose = new THREE.Mesh(new THREE.ConeGeometry(0.16, 0.56, 12), trimMat);
    nose.rotation.x = -Math.PI / 2;
    nose.position.set(0, 0.5, 1.03);

    const rearThruster = new THREE.Mesh(new THREE.CylinderGeometry(0.11, 0.16, 0.32, 14), trimMat);
    rearThruster.rotation.x = Math.PI / 2;
    rearThruster.position.set(0, 0.42, -1.02);
    ...
    const wheelF = new THREE.Mesh(new THREE.TorusGeometry(0.34, 0.065, 14, 34), wheelMat);
    wheelF.rotation.y = Math.PI / 2;
    wheelF.position.set(0, 0.28, 0.94);

    const tireF = new THREE.Mesh(new THREE.TorusGeometry(0.34, 0.04, 14, 34), tireMat);
    tireF.rotation.y = Math.PI / 2;
    tireF.position.copy(wheelF.position);

    const wheelR = wheelF.clone();
    wheelR.position.z = -0.9;
    const tireR = tireF.clone();
    tireR.position.z = -0.9;

    const wheelCoreF = new THREE.Mesh(new THREE.CylinderGeometry(0.05, 0.05, 0.08, 12), trimMat);
    wheelCoreF.rotation.z = Math.PI / 2;
    wheelCoreF.position.copy(wheelF.position);
    const wheelCoreR = wheelCoreF.clone();
    wheelCoreR.position.z = -0.9;

    const forkF = new THREE.Mesh(new THREE.CylinderGeometry(0.03, 0.03, 0.72, 8), trimMat);
    forkF.position.set(0, 0.5, 0.95);
    forkF.rotation.x = 0.14;
    const forkR = forkF.clone();
    forkR.position.z = -0.84;
    forkR.rotation.x = -0.1;

    const riderSuitMat = new THREE.MeshStandardMaterial({
      color: 0x101827,
      emissive: 0x0b1220,
    ...
    const torso = new THREE.Mesh(new THREE.CapsuleGeometry(0.14, 0.38, 8, 14), riderSuitMat);
    torso.position.set(0, 0.98, -0.06);
    torso.rotation.x = -0.34;

    const head = new THREE.Mesh(new THREE.SphereGeometry(0.12, 14, 12), riderSuitMat);
    head.position.set(0, 1.24, 0.2);

    const visor = new THREE.Mesh(new THREE.BoxGeometry(0.17, 0.09, 0.04), visorMat);
    visor.position.set(0, 1.24, 0.29);

    const shoulderL = new THREE.Mesh(new THREE.CapsuleGeometry(0.05, 0.24, 4, 8), riderSuitMat);
    shoulderL.position.set(-0.15, 1.0, 0.03);
    shoulderL.rotation.z = -0.55;
    shoulderL.rotation.x = -0.5;
    const shoulderR = shoulderL.clone();
    shoulderR.position.x = 0.15;
    shoulderR.rotation.z = 0.55;

    const accentL = new THREE.Mesh(new THREE.BoxGeometry(0.03, 0.14, 0.72), trimMat);
    accentL.position.set(-0.18, 0.54, 0.04);
    const accentR = accentL.clone();
    accentR.position.x = 0.18;
  ...
  function addNeonRims() {
    const rimMat = new THREE.MeshBasicMaterial({ color: 0x22d3ee, transparent: true, opacity: 0.34 });
    const rimF = new THREE.Mesh(new THREE.RingGeometry(0.31, 0.39, 28), rimMat);
    rimF.position.set(0, 0.29, 0.95);
    rimF.rotation.y = Math.PI / 2;
    const rimR = rimF.clone();
    rimR.position.z = -0.9;
    visuals.add(rimF, rimR);
  }

  clearVisuals();
  visuals.add(buildFallbackBike());
  addNeonRims();

  async function tryLoadPlayerModels() {
    ...
    const loadFirst = async (candidates) => {
    ...
    const bikeScene = await loadFirst(bikeCandidates);
    if (!bikeScene) {
      return;
    }

    const riderScene = await loadFirst(riderCandidates);

    const modelRoot = new THREE.Group();
    configureModel(bikeScene, 0.85, 0.12, Math.PI);
    modelRoot.add(bikeScene);

    if (riderScene) {
      configureModel(riderScene, 0.88, 0.88, Math.PI);
      riderScene.position.z -= 0.04;
      modelRoot.add(riderScene);
    }

    clearVisuals();
    visuals.add(modelRoot);
    addNeonRims();
  }

  tryLoadPlayerModels().catch(() => {
    // Keep fallback visuals if external models are unavailable.
  });
